 ...
 * run a build on a Bazel workspace.
 ...
 * <p>
 * Usage:
 * java -jar [path to jar file] [path to Bazel workspace]
 */
	...
	private static void printResult(List<BazelProblem> problems) {
		if (problems.size() == 0) {
			...
			for (BazelProblem problem : problems) {
				System.out.println(problem.getResourcePath()+":L"+problem.getLineNumber()+":  "+problem.getDescription());
			}
		}
	}
}
