 ...
 *
 * <p>
 * Handles a mixture of included and excluded target expressions.
 */
public final class TargetExpressionList {
    ...
    public static TargetExpressionList create(List<TargetExpression> targets) {
        return new TargetExpressionList(targets.stream().map(TargetData::new).collect(toImmutableList()), null);
    }

    static TargetExpressionList createWithTargetsDerivedFromDirectories(List<TargetExpression> targets,
            ProjectDirectoriesHelper directories) {
        return new TargetExpressionList(targets.stream().map(TargetData::new).collect(toImmutableList()), directories);
    }

    /**
     * The list of the project targets, in the reverse order to which they're passed to blaze, since later target
     * expressions override earlier ones.
     */
    ...
    private final ProjectDirectoriesHelper directories;

    private TargetExpressionList(ImmutableList<TargetData> projectTargets, ProjectDirectoriesHelper directories) {
    ...
    public ImmutableList<TargetExpression> getTargets() {
        return reversedTargets.reverse().stream().map(t -> t.originalExpression).collect(toImmutableList());
    }
