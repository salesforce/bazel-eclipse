 ...
 */
package com.salesforce.bazel.sdk.primitives;
...
public class PackagePathValidator {
    ...
    private static final CharMatcher ALLOWED_CHARACTERS_IN_PACKAGE_NAME =
            CharMatcher.inRange('0', '9').or(CharMatcher.inRange('a', 'z')).or(CharMatcher.inRange('A', 'Z'))
    ...
    static String validatePackageName(String packageName) {
        ...
        for (var i = len - 1; i >= -1; --i) {
            var c = (i >= 0) ? packageName.charAt(i) : '/';
            if (c == '/') {
                if (lastSlash) {
