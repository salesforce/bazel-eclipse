...
public final class JavaBlazeRules implements Kind.Provider {
    ...
    public Function<TargetIdeInfo, Kind> getTargetKindHeuristics() {
        return proto -> proto.hasJavaIdeInfo() || proto.hasJavaToolchainIdeInfo()
                ? Kind.Provider.create(proto.getKindString(), LanguageClass.JAVA, RuleType.UNKNOWN) : null;
    }

    @Override
    public ImmutableSet<Kind> getTargetKinds() {
        return Arrays.stream(RuleTypes.values()).map(e -> Kind.Provider.create(e.name, e.languageClass, e.ruleType))
