...
/**
 * Represents a corresponding protobuf {@link Message} from intellij_ide_info.proto or project_data.proto. Provides
 * utility methods to convert to and from a {@link Message} for easy serialization.
 */
public interface ProtoWrapper<P> {
    ...
    static <P, Q, R> R map(Iterable<P> iterable, Function<P, Q> function, Collector<Q, ?, R> collector) {
        return Streams.stream(iterable).map(function).collect(collector);
    }

    static <P, Q, R, S> ImmutableMap<Q, S> map(Map<P, R> map, Function<P, Q> keyFunction,
            Function<R, S> valueFunction) {
        return map.entrySet().stream().collect(
            ImmutableMap.toImmutableMap(e -> keyFunction.apply(e.getKey()), e -> valueFunction.apply(e.getValue())));
    }
    ...
    static <P, Q> ImmutableMap<P, Q> mapToProtos(Map<? extends ProtoWrapper<P>, ? extends ProtoWrapper<Q>> wrappers) {
