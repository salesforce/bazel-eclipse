...
public interface OutputArtifactParser {
  ...
  class LocalFileParser implements OutputArtifactParser {
    ...
    public OutputArtifact parse(
      ...
      try {
        Path path = new File(new URI(uri)).toPath();
        return new LocalFileOutputArtifact(
            path, getBlazeOutRelativePath(file, configurationMnemonic), configurationMnemonic);
    ...
    private static String getBlazeOutRelativePath(
      ...
      if (pathPrefixList.size() <= 1) {
        throw new IllegalArgumentException("Invalid output from BuildEventStream. No longer support: " + pathPrefixList);
      }
