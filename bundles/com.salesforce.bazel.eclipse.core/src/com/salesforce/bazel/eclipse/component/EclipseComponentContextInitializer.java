...
import com.salesforce.bazel.eclipse.config.BazelEclipseAspectLocation;
...
public class EclipseComponentContextInitializer implements IComponentContextInitializer {
    ...
    public void initialize() {
        ...
        BazelAspectLocation bazelAspectLocation = new BazelEclipseAspectLocation();

        var commandBuilder = new ShellCommandBuilder(consoleFactory, new EclipseShellEnvironment(eclipsePrefsHelper));

        ComponentContext.getInstance().initialize(projectManager, resourceHelper, javaCoreHelper, osStrategy,
            configManager, eclipsePrefsHelper, bazelAspectLocation, commandBuilder, consoleFactory);
    }
}
