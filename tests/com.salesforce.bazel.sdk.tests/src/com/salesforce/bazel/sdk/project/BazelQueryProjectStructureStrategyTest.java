...
public class BazelQueryProjectStructureStrategyTest {
        ...
        protected SplitSourcePath splitSourcePath(File packageDir, String srcPath) {
            ...
            for (String candidate : sourcePathsForThisTest) {
                if (srcPath.startsWith(candidate + File.separator)) {
                    var split = new SplitSourcePath();
                    var dirPathLength = candidate.length();
    ...
    public void testHappyPath() {
        ...
        var strategy = new TestBazelQueryProjectStructureStrategy();
        ...
        // run the test
        var relPath = FSPathHelper.osSeps("projects/libs/apple");
        var packageLocation = new InMemoryPackageLocation(relPath);
    ...
    public void testMultipleSourceDirs() {
        ...
        var relPath = FSPathHelper.osSeps("projects/libs/apple");
        var packageLocation = new InMemoryPackageLocation(relPath);
        var structure = strategy.doStructureAnalysis(bazelWorkspace, packageLocation, nullCommandRunner);
